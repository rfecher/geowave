...
public class SpatialTemporalQueryExample
{
	...
	private void executePolygonAndTimeRangeQuery()
		...
		final SpatialTemporalQuery query = new SpatialTemporalQuery(
				...
				org.locationtech.geowave.adapter.vector.util.FeatureGeometryUtils.buffer(
		...
		final CloseableIterator<SimpleFeature> iterator = dataStore.query(new QueryBuilderImpl<SimpleFeature>()
				.constraints(
						query)
				.build());
		...
		// TODO do we care to provide multi-time range in QueryBuilder
		...
		final CloseableIterator<SimpleFeature> iterator2 = dataStore.query(new QueryBuilderImpl<SimpleFeature>()
				.addType(
						adapter)
				.index(
						index)
