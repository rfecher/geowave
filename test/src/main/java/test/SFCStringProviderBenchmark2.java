...
public class SFCStringProviderBenchmark2 {
  ...
  public static void main(final String[] args) throws IOException {
    ...
    final SFCDimensionDefinition[] defs = new SFCDimensionDefinition[numDimensions*2];
    for (int i = 0; i < numDimensions*2; i++) {
      defs[i] =
          new SFCDimensionDefinition(
              ...
              Math.floorDiv(63, numDimensions*2));
    }
    final HilbertSFC sfc = new HilbertSFC(defs);
    final int ngram = 2;
    int id = 0;
    try (final NGramTokenizer tokenizer = new NGramTokenizer(ngram, ngram)) {
      ...
      for (int i = 3; i <= maxSearchString; i++) {
        ...
        while (qIterator.hasNext()) {
          ...
          while (tokenizer.incrementToken()) {
            final String term = charTermAttribute.toString();
            final NumericData[] dataPerDimension = new NumericData[numDimensions*2];
            for (int d = 0; d < numDimensions*2; d+=2) {
              final int hashCode = hashTerm(term, d);
              dataPerDimension[d] = new NumericRange(Integer.MIN_VALUE,hashCode);
              dataPerDimension[d+1] = new NumericRange(hashCode,Integer.MAX_VALUE);
            }
            final RangeDecomposition range =
                sfc.decomposeRange(new BasicNumericDataset(dataPerDimension),true, 100);
