...
public class SpatialTemporalQueryIT
{
	...
	private static final Index DAY_INDEX = new SpatialTemporalIndexBuilder()
			...
			.setPeriodicity(
					Unit.DAY)
			.createIndex();
	private static final Index MONTH_INDEX = new SpatialTemporalIndexBuilder()
			.setPartitionStrategy(
			...
			.setPeriodicity(
					Unit.MONTH)
			.createIndex();
	private static final Index YEAR_INDEX = new SpatialTemporalIndexBuilder()
			.setPartitionStrategy(
					PartitionStrategy.HASH)
			.setNumPartitions(
					10)
	...
	public void initSpatialTemporalTestData()
		...
		dataStore
				.addIndex(
						...
						MONTH_INDEX,
						DAY_INDEX);
		final Writer timeWriters = dataStore
				.createWriter(
						timeStampAdapter.getTypeName());
		...
		dataStore
				.addIndex(
						...
						YEAR_INDEX,
						MONTH_INDEX);
		final Writer rangeWriters = dataStore
				.createWriter(
						timeRangeAdapter.getTypeName());
		...
		try {
			for (int day = cal
					.getActualMinimum(
							Calendar.DAY_OF_MONTH); day <= cal
									.getActualMaximum(
											Calendar.DAY_OF_MONTH); day++) {
				final double ptVal = ((((day + 1.0) - cal
						.getActualMinimum(
				...
				featureBuilder
						.add(
								cal.getTime());
				final SimpleFeature feature = featureBuilder
						.buildFeature(
								"day:" + day);
				timeWriters
						.write(
			...
			for (int month = cal
				...
				final double ptVal = ((((month + 1.0) - cal
						.getActualMinimum(
								Calendar.MONTH))
						/ ((cal
								.getActualMaximum(
			...
			featureTimeRangeBuilder
					.add(
							cal.getTime());
			feature = featureTimeRangeBuilder
					.buildFeature(
							"outlier1timerange");
			timeWriters
					.write(
	...
	private static void ingestTimeRangeData(
		...
		featureTimeRangeBuilder
				.add(
						cal.getTime());
		SimpleFeature feature = featureTimeRangeBuilder
				.buildFeature(
						name + ":fullrange");
		writer
				.write(
	...
	private void testQueryGivenDateRange(
			...
			throws CQLException,
			IOException {
		final String cqlPredicate = "BBOX(\"geo\",-1,-1,1,1) AND \""
				+ startTimeAttribute + "\" <= '" + CQL_DATE_FORMAT
						.format(
								endOfQuery)
				+ "' AND \"" + endTimeAttribute + "\" >= '" + CQL_DATE_FORMAT
						.format(
								startOfQuery)
				+ "'";
		final Set<String> fidResults = new HashSet<>();
		try (CloseableIterator<SimpleFeature> it = (CloseableIterator)dataStore
				.query(bldr.constraints(
	...
	public void testQueryMultipleBinsDay()
			throws IOException,
			CQLException {VectorQueryBuilder bldr = VectorQueryBuilder.newBuilder(); 
			bldr
				.indexName(
						DAY_INDEX.getName());
