		...
		AbstractGeoWaveIT
{
	...
	public static void reportTestStart() {
		...
		LOGGER
				.warn(
						"*  RUNNING GeoWaveVectorSerializationIT *");
		LOGGER
				.warn(
						"*                                       *");
		LOGGER
	...
	public static void reportTestFinish() {
		...
		LOGGER
				.warn(
						"* FINISHED GeoWaveVectorSerializationIT *");
		LOGGER
				.warn(
	...
	public void testFeatureSerialization()
		...
		final SimpleFeature sf = serBuilder
				.buildFeature(
						"343");
		geowaveStore
				.addType(
		...
		try (Writer writer = geowaveStore
				.createWriter(
						serAdapter.getTypeName())) {
			writer
					.write(
				...
				((Geometry) args
						.get(
								Geometry.class))
										.buffer(
												0.5d));
		try (final CloseableIterator<?> iter = geowaveStore
				.query(QueryBuilder.newBuilder().constraints(q).build())) {
			...
			while (iter.hasNext()) {
				...
				for (final Property p : isFeat.getProperties()) {
					final Object before = args
							.get(
									p.getType().getBinding());
					final Object after = isFeat
							.getAttribute(
