...
public class HistogramStatistics extends
  ...
  public HistogramStatistics(final HistogramConfig histogramConfig) {
    this(null, histogramConfig);
  }

  public HistogramStatistics(final Short adapterId, final HistogramConfig histogramConfig) {
  ...
  public byte[] toBinary() {
    ...
    VarintUtils.writeUnsignedInt(configBinary.length, buf);
    ...
    if ((statistics != null) && (statistics instanceof HistogramStatistics)) {
      ...
      for (final Resolution res : resolutions) {
        final javax.media.jai.Histogram otherHistogram =
            ((HistogramStatistics) statistics).getHistogram(res);
  ...
  protected Object resultsValue() {
    ...
    for (final Entry<Resolution, javax.media.jai.Histogram> e : histograms.entrySet()) {
      ...
      h.put("resolution", e.getKey().getResolutionPerDimension());
      h.put("value", e.getValue().toString());
      map.put("histogram", h);
    }
    return map;
  }
