...
public class VisibilityConfigurationTest {
  ...
  public void test() {
    ...
    schema.getUserData().put("visibilityManagerClass", TestExtension.class.getName());

    final VisibilityConfiguration config = new VisibilityConfiguration(schema);

    assertEquals("pid", config.getAttributeName());
    assertEquals(TestExtension.class, config.getManager().getClass());
