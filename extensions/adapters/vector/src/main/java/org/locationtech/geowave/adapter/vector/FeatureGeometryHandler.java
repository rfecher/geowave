...
public class FeatureGeometryHandler
  ...
  public FeatureGeometryHandler(final AttributeDescriptor geometryAttrDesc) {
    this(geometryAttrDesc, null);
  }

  public FeatureGeometryHandler(final AttributeDescriptor geometryAttrDesc,
      final FieldVisibilityHandler<SimpleFeature, Object> visibilityHandler) {
    nativeGeometryHandler = new FeatureAttributeHandler(geometryAttrDesc);
    this.visibilityHandler = visibilityHandler;
    nativeFieldNames = new String[] {nativeGeometryHandler.getFieldName()};
  }
  ...
  public GeometryWrapper toIndexValue(final SimpleFeature row) {
    ...
    if (visibilityHandler != null) {
      visibility =
          visibilityHandler.getVisibility(row, nativeGeometryHandler.getFieldName(), geometry);
    } else {
      visibility = new byte[] {};
    }
    return new GeometryWrapper(geometry, visibility);
  }
