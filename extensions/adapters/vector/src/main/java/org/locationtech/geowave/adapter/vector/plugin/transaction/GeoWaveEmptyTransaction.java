...
public class GeoWaveEmptyTransaction extends AbstractTransactionManagement
   ...
   */
  public void modify(String fid, SimpleFeature original, SimpleFeature updated) throws IOException {
    // point move?
    if (!updated.getBounds().equals(original.getBounds())) {
      this.components.remove(original, this);
      this.components.writeCommit(updated, new GeoWaveEmptyTransaction(components));
    } else {
      this.components.writeCommit(updated, new GeoWaveEmptyTransaction(components));
    }
  ...
  public void add(String fid, SimpleFeature feature) throws IOException {
    ...
    if (feature.getUserData().containsKey(Hints.PROVIDED_FID)) {
      String providedFid = (String) feature.getUserData().get(Hints.PROVIDED_FID);
