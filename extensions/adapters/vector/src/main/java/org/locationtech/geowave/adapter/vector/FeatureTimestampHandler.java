...
/**
 * This class handles the internal responsibility of persisting single timestamp instants based on a
 * temporal attribute (a Java binding class of Date or Calendar for an attribute)to and from a
 * GeoWave common index field for SimpleFeature data.
 */
public class FeatureTimestampHandler implements IndexFieldHandler<SimpleFeature, Time, Object> {
  ...
  public FeatureTimestampHandler(final AttributeDescriptor timestampAttrDesc) {
    this(timestampAttrDesc, null);
  }
  ...
  public FeatureTimestampHandler(
    ...
    nativeTimestampHandler = new FeatureAttributeHandler(timestampAttrDesc);
    this.visibilityHandler = visibilityHandler;
    nativeFieldNames = new String[] {nativeTimestampHandler.getFieldName()};
  }
  ...
  public Time toIndexValue(final SimpleFeature row) {
    ...
    if (visibilityHandler != null) {
      visibility =
          visibilityHandler.getVisibility(row, nativeTimestampHandler.getFieldName(), object);
    } else {
      visibility = new byte[] {};
    }
    return new Timestamp(TimeUtils.getTimeMillis(object), visibility);
  }
