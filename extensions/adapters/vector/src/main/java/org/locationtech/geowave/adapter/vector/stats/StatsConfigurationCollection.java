 ...
 */
public class StatsConfigurationCollection implements java.io.Serializable, Persistable {

  private static final long serialVersionUID = -4983543525776889248L;

  private static final Logger LOGGER = LoggerFactory.getLogger(StatsConfigurationCollection.class);
  ...
  public static class SimpleFeatureStatsConfigurationCollection
    ...
    public SimpleFeatureStatsConfigurationCollection(final SimpleFeatureType type) {
      super();
    ...
    public void setAttConfig(final Map<String, StatsConfigurationCollection> attConfig) {
    ...
    public byte[] toBinary() {
      int size = 0;
      final List<byte[]> entries = new ArrayList<>(attConfig.size());
      ...
      VarintUtils.writeUnsignedInt(attConfig.size(), buf);
