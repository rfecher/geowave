...
public class RowRead {
  ...
  protected RowRead(final PreparedStatement preparedRead, final CassandraOperations operations,
      final byte[] partitionKey, final byte[] sortKey, final Short internalAdapterId) {
  ...
  public CassandraRow result() {
    if ((partitionKey != null) && (sortKey != null)) {
      ...
      boundRead.set(CassandraField.GW_SORT_KEY.getBindMarkerName(), ByteBuffer.wrap(sortKey),
          ByteBuffer.class);
      boundRead.set(CassandraField.GW_ADAPTER_ID_KEY.getBindMarkerName(), internalAdapterId,
          TypeCodec.smallInt());
      boundRead.set(CassandraField.GW_PARTITION_ID_KEY.getBindMarkerName(),
          ByteBuffer.wrap(partitionKey), ByteBuffer.class);
