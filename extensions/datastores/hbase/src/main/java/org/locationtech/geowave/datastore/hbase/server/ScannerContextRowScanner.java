...
/**
 * This uses reflection to access package private methods and variables expected to be within
 * ScannerContext. This access mimics how whole row filtering is accomplished within HBase internal
 * scanner code, and similarly, rows are necessary to be aggregated together for GeoWave ServerSide
 * Operations. Using similar packaging to gain access to package private members was insufficient
 * due to how HBase Coprocessor classloading special cases the loading of HBase packages.
 * Alternatively, it may be sufficient to use NoLimitScannerContext.getInstance() at a higher level
 * than the row scanner, but no implementation of this potential alternative has been as well
 * tested.
 */
public class ScannerContextRowScanner implements RowScanner {
  private static final Logger LOGGER = Logger.getLogger(ScannerContextRowScanner.class);
  ...
  public ScannerContextRowScanner(final InternalScanner scanner, final List<Cell> cells,
      final ScannerContext scannerContext, final Scan scan) {
  ...
  private boolean partialResultFormed() {
    final NextState state = getReflectionInstance().getScannerState(scannerContext);
    return (state == NextState.SIZE_LIMIT_REACHED_MID_ROW)
        || (state == NextState.TIME_LIMIT_REACHED_MID_ROW);
  }
  ...
  public List<Cell> nextCellsInRow() throws IOException {
    ...
    getReflectionInstance().reset(scannerContext);
    done = !scanner.next(cells, scannerContext);
    return cells;
  }
