...
public class AccumuloSecondaryIndexUtils {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(AccumuloSecondaryIndexUtils.class);

  /**
   * Decodes an Accumulo key-value pair from the result of a secondary index scan back into the
   * original type using the given data adapter
   *
   * @param key
   * @param value
   * @param adapter
  ...
  public static <T> T decodeRow(
    ...
    for (final Entry<Key, Value> entry : rowMapping.entrySet()) {
      ...
      if (dataId == null) {
        dataId = StringUtils.stringToBinary(dataIdString);
      }
