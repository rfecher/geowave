...
public class AccumuloSecondaryIndexDataStore extends BaseSecondaryIndexDataStore {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(AccumuloSecondaryIndexDataStore.class);
  ...
  @Override
  protected RowWriter getWriter(final String secondaryIndexName) {
  ...
  private Scanner getScanner(final String secondaryIndexId, final String... visibility) {
    Scanner scanner = null;
    try {
      scanner = accumuloOperations.createScanner(secondaryIndexId, visibility);
    } catch (final TableNotFoundException e) {
      LOGGER.error("Could not obtain batch scanner", e);
    }
    return scanner;
  }

  private Collection<Range> getScanRanges(final List<ByteArrayRange> ranges) {
