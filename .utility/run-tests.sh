...
if [ "$PYTHON_BUILD" == "true" ]; then
  ...
  if [ "$IT_ONLY" == "true" ]; then
    ...
    tar -xzf ./hadoop-3.3.0.tar.gz hadoop-3.3.0/lib/native/
    export LD_LIBRARY_PATH=$(pwd)/hadoop-3.3.0/lib/native/
    mvn verify -am -pl test -Dtest=SkipUnitTests -Dfindbugs.skip -Dspotbugs.skip -DfailIfNoTests=false -P $MAVEN_PROFILES
  else
