...
if [ "$PYTHON_BUILD" == "true" ]; then
  ...
  if [ "$IT_ONLY" == "true" ]; then
    ...
    mvn -q verify -am -pl test --batch-mode -Dtest=SkipUnitTests -Dfindbugs.skip -Dspotbugs.skip -DfailIfNoTests=false -P $MAVEN_PROFILES
  else
    echo -e "Running unit tests only w/ verify...\n"
    mvn -q verify --batch-mode -Dformatter.action=validate -P $MAVEN_PROFILES
  fi
fi
