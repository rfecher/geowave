...
public class LongitudeDefinitionTest {
  ...
  public void testNormalizeDateLineCrossingEast() {
    ...
    for (int i = 0; i < binRange.length; i++) {
      ...
      Assert.assertEquals(expectedBinRanges[i].getNormalizedMin(), binRange[i].getNormalizedMin(),
          DELTA);

      Assert.assertEquals(expectedBinRanges[i].getNormalizedMax(), binRange[i].getNormalizedMax(),
          DELTA);
    }
  }
