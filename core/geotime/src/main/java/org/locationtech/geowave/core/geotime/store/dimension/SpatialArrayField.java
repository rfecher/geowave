...
public class SpatialArrayField extends ArrayField<GeometryWrapper>
  ...
  public SpatialArrayField(final NumericDimensionField<GeometryWrapper> elementField) {
    super(elementField);
    reader = new ArrayWrapperReader<GeometryWrapper>(new ArrayReader(elementField.getReader()));
    writer = new ArrayWrapperWriter<GeometryWrapper>(
        new VariableSizeObjectArrayWriter(elementField.getWriter()));
  }
