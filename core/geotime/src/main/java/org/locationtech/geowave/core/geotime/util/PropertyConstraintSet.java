...
public class PropertyConstraintSet {
  ...
  public PropertyConstraintSet(final FilterableConstraints constraint) {
    add(constraint, true);
  }
  ...
  public List<FilterableConstraints> getConstraintsFor(final String[] fieldIds) {
  ...
  public List<ByteArrayRange> getRangesFor(final SecondaryIndexImpl<?> index) {
    ...
    if (c != null) {
      ...
      result.addAll(index.getIndexStrategy().getQueryRanges(c).getCompositeQueryRanges());
    }
    return result;
  }

  public List<QueryFilter> getFiltersFor(final SecondaryIndexImpl<?> index) {
  ...
  public void add(final FilterableConstraints constraint, final boolean intersect) {
    ...
    if (constraintsForId == null) {
      ...
      constraints.put(id, constraintsForId.intersect(constraint));
    } else {
      constraints.put(id, constraintsForId.union(constraint));
    }
  }

  public void intersect(final PropertyConstraintSet set) {
    for (final Map.Entry<String, FilterableConstraints> entry : set.constraints.entrySet()) {
      add(entry.getValue(), true);
    }
  }

  public void union(final PropertyConstraintSet set) {
    for (final Map.Entry<String, FilterableConstraints> entry : set.constraints.entrySet()) {
      add(entry.getValue(), false);
    }
  }

  public FilterableConstraints getConstraintsByName(final String id) {
