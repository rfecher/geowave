...
public class TimeArrayField extends ArrayField<Time>
  ...
  public TimeArrayField(final NumericDimensionField<Time> elementField) {
    super(elementField);
    reader = new ArrayWrapperReader<>(new ArrayReader<>(elementField.getReader()));
    writer =
        new ArrayWrapperWriter<Time>(new VariableSizeObjectArrayWriter(elementField.getWriter()));
  }
