		...
		TimeRangeAggregation<FieldNameParam, SimpleFeature>
{
	private final static Logger LOGGER = LoggerFactory
			.getLogger(
					VectorTimeRangeAggregation.class);
	...
	protected Interval getInterval(
		...
		if ((desc.getStartRange() != null) && (desc.getEndRange() != null)) {
			final Object start = entry
					.getAttribute(
							desc.getStartRange().getName());
			final Object end = entry
					.getAttribute(
							desc.getStartRange().getName());
			if ((start == null) || (end == null)) {
				LOGGER
						.warn(
								"start or end value is null, ignoring feature");
				return null;
			}
			...
			final Object time = entry
					.getAttribute(
							desc.getTime().getName());
			if ((time == null)) {
				LOGGER
						.warn(
								"time attribute value is null, ignoring feature");
				return null;
			}
		...
		LOGGER
				.error(
						"time field not found for type '" + entry.getFeatureType().getTypeName()
