...
public class IndexLoaderTest {
  ...
  public void testMultipleIndices() throws IOException {
    ...
    final File props = File.createTempFile("IndexLoaderTest.testMultipleIndices", "props");
    props.deleteOnExit();
    params.getContext().put(ConfigOptions.PROPERTIES_FILE_CONTEXT, props);
    ...
    Assert.assertTrue("Unable to load multiple indices from property file",
        loader.loadFromConfig(props));
    final List<IndexPluginOptions> options = loader.getLoadedIndexes();
    Assert.assertEquals("Given multiple indices an incorrect number of indices loaded", 2,
        options.size());
  }
  ...
  public void testIndexGroup() throws Exception {
    ...
    addIndex.setParameters("index1");
    final ManualOperationParams params = new ManualOperationParams();
    final File props = File.createTempFile("IndexLoaderTest.testIndexGroup", "props");
    props.deleteOnExit();
    params.getContext().put(ConfigOptions.PROPERTIES_FILE_CONTEXT, props);
    addIndex.prepare(params);
    addIndex.execute(params);
    ...
    addIndexGroup.setParameters("indexGroup1", "index1,index2");
    addIndexGroup.prepare(params);
    addIndexGroup.execute(params);
    final IndexLoader loader = new IndexLoader("indexGroup1");
    Assert.assertTrue("Unable to load index groups from property file",
        loader.loadFromConfig(props));
    final List<IndexPluginOptions> options = loader.getLoadedIndexes();
    Assert.assertEquals("Given a single index group an incorrect number of indices loaded", 2,
        options.size());
  }

  @Test
  public void testMutlipleIndexGroupsAndIndices() throws Exception {
    final AddIndexCommand addIndex = new AddIndexCommand();
    addIndex.setType("test");
    addIndex.setParameters("index1");
    final ManualOperationParams params = new ManualOperationParams();
    final File props =
        File.createTempFile("IndexLoaderTest.testMutlipleIndexGroupsAndIndices", "props");
    props.deleteOnExit();
    params.getContext().put(ConfigOptions.PROPERTIES_FILE_CONTEXT, props);
    addIndex.prepare(params);
    addIndex.execute(params);
    ...
    addIndexGroup.prepare(params);
    addIndexGroup.execute(params);
    addIndexGroup.setParameters("indexGroup2", "index3,index2");
    addIndexGroup.prepare(params);
    addIndexGroup.execute(params);
    final IndexLoader loader = new IndexLoader("indexGroup1,indexGroup2,index4,index2");
    Assert.assertTrue("Unable to load combination of index groups and indices from property file",
        loader.loadFromConfig(props));
    final List<IndexPluginOptions> options = loader.getLoadedIndexes();
    Assert.assertEquals(
