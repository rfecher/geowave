 ...
 * IndexGroupPluginOptions based on the name of the index (not the type). It will load from config
 * file.
 */
public class IndexLoader {
  ...
  /** Constructor */
  public IndexLoader(final String indexName) {
    this.indexName = indexName;
  }

  /**
   * Attempt to find an index group or index name in the config file with the given name.
   *
   * @param configFile
   * @return
   */
  ...
  public boolean loadFromConfig(final Properties props) {
    ...
    for (final String index : indices) {
      ...
      final IndexGroupPluginOptions indexGroupOptions = loadIndexGroupPluginOptions(props, index);
  ...
  public void setLoadedIndices(final Map<String, IndexPluginOptions> loadedIndexes) {
