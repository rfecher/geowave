...
public class IndependentAdapterIndexWriter<T> implements Writer<T> {
  ...
  public IndependentAdapterIndexWriter(final IndexDependentDataAdapter<T> adapter,
      final Index index, final Writer<T> writer) {
    super();
    this.writer = writer;
    this.index = index;
  ...
  @Override
  public InsertionIds write(final T entry, final VisibilityWriter<T> feldVisibilityWriter) {
    ...
    while (indexedEntries.hasNext()) {
      final InsertionIds ids = writer.write(indexedEntries.next(), feldVisibilityWriter);
