		...
		DataStore
{
	...
	private <T> Writer<T> createWriter(
		...
		final Writer<T>[] writers = new Writer[baseOptions.isSecondaryIndexing() ? indices.length +1 : indices.length];
		...
		if (baseOptions.isSecondaryIndexing()) {
			final DataStoreCallbackManager callbackManager = new DataStoreCallbackManager(
					...
					secondaryIndexDataStore,
					true);
			final List<IngestCallback<T>> callbacks = Collections
					.singletonList(
							callbackManager
									.getIngestCallback(
											adapter,
	...
	private void internalAddIndices(
		...
		if (adapter.getAdapter() instanceof InitializeWithIndicesDataAdapter) {
			if (((InitializeWithIndicesDataAdapter) adapter.getAdapter())
					.init(
							indices)
					&& updateAdapter) {
				adapterStore
