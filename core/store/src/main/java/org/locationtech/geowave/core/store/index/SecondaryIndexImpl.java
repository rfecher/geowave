...
public class SecondaryIndexImpl<T>
  ...
  /**
   * Compare this object to the one passed as parameter to see if same object, same class and that
   * id is the same.
   */
  @Override
  ...
  public byte[] toBinary() {
    ...
    final byte[] secondaryIndexTypeBinary =
        StringUtils.stringToBinary(secondaryIndexType.getValue());
    ...
    if (handlePartials) {
      ...
      for (final String partialFieldName : partialFieldNames) {
        ...
        VarintUtils.writeUnsignedInt(partialFieldBytes.length, allPartials);
    ...
    VarintUtils.writeUnsignedInt(persistablesBinary.length, buf);
    VarintUtils.writeUnsignedInt(handlePartials ? partialFieldNames.size() : 0, buf);
    buf.put(indexStrategyBinary);
    buf.put(fieldIdBinary);
    buf.put(secondaryIndexTypeBinary);
    buf.put(persistablesBinary);
    if (handlePartials) {
      VarintUtils.writeUnsignedInt(partialsLength, buf);
  ...
  public void fromBinary(final byte[] bytes) {
    ...
    secondaryIndexType =
        SecondaryIndexType.valueOf(StringUtils.stringFromBinary(secondaryIndexTypeBinary));
