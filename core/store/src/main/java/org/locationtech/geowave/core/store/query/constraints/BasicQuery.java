...
public class BasicQuery implements QueryConstraints {
  ...
  public static class ConstraintData {
    ...
    public ConstraintData merge(final ConstraintData cd) {
      ...
      return new ConstraintData(new NumericRange(Math.min(cd.range.getMin(), range.getMin()),
          Math.max(cd.range.getMax(), range.getMax())), false); // TODO: ideally, this would be set
      // based on some
      // logic
    }
  ...
  public static class Constraints {
    ...
    public Constraints(final ConstraintSet constraintSet) {
      constraintsSets.add(constraintSet);
    }

    public Constraints(final List<ConstraintSet> constraintSets) {
      constraintsSets.addAll(constraintSets);
    }

    public Constraints merge(final Constraints constraints) {
      return merge(constraints.constraintsSets);
    }

    public Constraints merge(final List<ConstraintSet> otherConstraintSets) {

      if (otherConstraintSets.isEmpty()) {
        return this;
      } else if (isEmpty()) {
        return new Constraints(otherConstraintSets);
      }
    ...
    public boolean matches(final Constraints constraints) {
      if (constraints.isEmpty() != isEmpty()) {
        return false;
      }
  ...
  public List<QueryFilter> createFilters(final Index index) {
    ...
    for (final ConstraintSet constraint : constraints.constraintsSets) {
      final QueryFilter filter = constraint.createFilter(index, this);
      if (filter != null) {
        filters.add(filter);
      }
    }
