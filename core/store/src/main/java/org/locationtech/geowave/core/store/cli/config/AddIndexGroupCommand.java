...
public class AddIndexGroupCommand extends ServiceEnabledCommand<String> {
  ...
  /**
   * Add rest endpoint for the addIndexGroup command. Looks for POST params with keys 'key' and
   * 'value' to set.
   *
   * @return none
   * @throws Exception
   ...
   * @throws Exception
   */
  private String addIndexGroup(final OperationParams params) throws Exception {
    ...
    final String[] indexes = parameters.get(1).split(",");

    // Make sure the existing group doesn't exist.
    final IndexGroupPluginOptions groupOptions = new IndexGroupPluginOptions();
    if (groupOptions.load(existingProps, getNamespace())) {
      throw new DuplicateEntryException("That index group already exists: " + getPluginName());
    }
    ...
    for (int i = 0; i < indexes.length; i++) {
      ...
      if (!options.load(existingProps, IndexPluginOptions.getIndexNamespace(indexes[i]))) {
        throw new TargetNotFoundException("That index does not exist: " + indexes[i]);
      }
      groupOptions.getDimensionalityPlugins().put(indexes[i], options);
    }

    // Save the group
    groupOptions.save(existingProps, getNamespace());
