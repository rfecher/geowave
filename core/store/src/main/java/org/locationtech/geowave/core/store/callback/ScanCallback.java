...
public interface ScanCallback<T, R extends GeoWaveRow> {
   ...
   *
   * <p>
   * Without or without de-duplication, row ids are not consolidate, thus each entry only contains
   * one row id. If the entry is not de-dupped, then the entry this method is called for each
   * duplicate, each with a different row id.
   *
