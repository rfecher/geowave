...
/**
 * This is used internally to translate DataStore rows into native objects (using the appropriate
 * data adapter). It also performs any client-side filtering. It will peek at the next entry in the
 * wrapped iterator to always maintain a reference to the next value.
 *
 * @param <T> The type for the entry
 */
public abstract class EntryIteratorWrapper<T> implements Iterator<T> {
