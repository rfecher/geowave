 ...
 *
 * @param <T> The type of entity being indexed
 */
public class SecondaryIndexDataManager<T>
    implements Closeable, IngestCallback<T>, DeleteCallback<T, GeoWaveRow> {
  ...
  public void entryCallback(final T entry, final boolean delete, final GeoWaveRow... kvs) {
    ...
    for (final SecondaryIndexImpl<T> secondaryIndex : adapter.getSupportedSecondaryIndices()) {
      ...
      final int position =
          adapter.getPositionOfOrderedField(primaryIndexModel, indexedAttributeFieldName);
      Object fieldValue = null;
      byte[] visibility = null;
      // find the field value and deserialize it
      for (final GeoWaveValue v : kvs[0].getFieldValues()) {
        if (BitmaskUtils.getFieldPositions(v.getFieldMask()).contains(position)) {
          final byte[] fieldSubsetBitmask = BitmaskUtils.generateCompositeBitmask(position);

          final byte[] byteValue =
              BitmaskUtils.constructNewValue(v.getValue(), v.getFieldMask(), fieldSubsetBitmask);
          fieldValue = adapter.getReader(indexedAttributeFieldName).readField(byteValue);
            ...
            for (final GeoWaveValue value : kvs[0].getFieldValues()) {
              ...
              if (fieldSubsetBitmask != null) {
                final byte[] newBitmask =
                    BitmaskUtils.generateANDBitmask(fieldMask, fieldSubsetBitmask);
