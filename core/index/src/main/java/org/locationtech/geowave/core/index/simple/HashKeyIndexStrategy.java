 ...
 *
 * <p>
 * Before using this class, one should consider balancing options for the specific data store. Can
 * one pre-split using a component of another index strategy (e.g. bin identifier)? How about ingest
 * first and then do major compaction?
 *
 * <p>
 ...
 *
 * <p>
 * This class should be used with a CompoundIndexStrategy. In addition, tablets should be pre-split
 * on the number of prefix IDs. Without splits, the splits are at the mercy of the Big Table servers
 * default. For example, Accumulo fills up one tablet before splitting, regardless of the
 * partitioning.
 *
 * <p>
 * The key set size does not need to be large. For example, using two times the number of tablet
 * servers (for growth) and presplitting, two keys per server. The default is 3.
 *
 * <p>
 * There is a cost to using this approach: queries must span all prefixes. The number of prefixes
...
public class HashKeyIndexStrategy
  ...
  public Set<ByteArray> getInsertionPartitionKeys(final MultiDimensionalNumericData insertionData) {
    final long hashCode = Math.abs(hashCode(insertionData.getMaxValuesPerDimension(),
