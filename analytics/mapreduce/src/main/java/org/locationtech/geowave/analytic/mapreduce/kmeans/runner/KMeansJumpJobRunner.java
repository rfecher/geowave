...
public class KMeansJumpJobRunner extends MapReduceJobController implements ClusteringRunner {
  ...
  public KMeansJumpJobRunner() {
    ...
    // child runners
    init(new MapReduceJobRunner[] {kmeansRunner, jumpRunner,},
  ...
  public int run(final Configuration configuration, final PropertyManagement propertyManagement)
    ...
    propertyManagement.copy(CentroidParameters.Centroid.DATA_TYPE_ID,
        SampleParameters.Sample.DATA_TYPE_NAME);

    propertyManagement.copy(CentroidParameters.Centroid.INDEX_NAME,
    ...
    final String currentBatchId = propertyManagement
        .getPropertyAsString(GlobalParameters.Global.BATCH_ID, UUID.randomUUID().toString());

    try {

      final NumericRange rangeOfIterations = propertyManagement
      ...
      final Class<AnalyticItemWrapperFactory> analyticItemWrapperFC =
          propertyManagement.getPropertyAsClass(CentroidParameters.Centroid.WRAPPER_FACTORY_CLASS,
  ...
  public Collection<ParameterEnum<?>> getParameters() {
    ...
    params.addAll(Arrays.asList(new ParameterEnum<?>[] {JumpParameters.Jump.RANGE_OF_CENTROIDS,
        JumpParameters.Jump.KPLUSPLUS_MIN, ClusteringParameters.Clustering.MAX_REDUCER_COUNT,
