...
public class SqlQueryRunner
{
	...
	public Dataset<Row> run()
		...
		final Pattern stringLit = Pattern
				.compile(
						"(?:\\'|\\\").*?(?:\\'|\\\")");
		final Matcher m = stringLit
				.matcher(
						sql);
		final String cleanedSql = m
				.replaceAll(
						"");
		LOGGER
				.debug(
		...
		if (!cleanedSql
			...
			final JsonElement jElement = gsonParser
					.parse(
							plan.prettyJson());
			if (jElement.isJsonArray()) {
				...
				for (int iObj = 0; iObj < size; iObj++) {
					final JsonElement childElement = jArray
							.get(
									iObj);
					if (childElement.isJsonObject()) {
						final JsonObject jObj = childElement.getAsJsonObject();
						final String objClass = jObj
								.get(
										"class")
								.getAsString();
						if (Objects
							...
							final JsonElement conditionElements = jObj
									.get(
											"condition");
		...
		if (extractedPredicates.size() == 1) {
			...
			final Pattern whereDetect = Pattern
					.compile(
							"(?i)(\"[^\"]*\"|'[^']*')|(\\bWHERE\\b)");
			final Pattern andOrDetect = Pattern
					.compile(
							"(?i)(\"[^\"]*\"|'[^']*')|(\\bAND|OR\\b)");
			final Pattern orderGroupDetect = Pattern
					.compile(
			...
			final int whereStart = filterStart
					.start(
							2);
			...
			final ExtractedGeomPredicate pred = extractedPredicates
					.get(
							0);
			// Parse filter string for predicate location
			final int functionPos = filterClause
					.indexOf(
							pred.predicateName);
			final int funcArgStart = filterClause
					.indexOf(
							"(",
							functionPos);
			final int funcArgEnd = filterClause
					.indexOf(
							")",
							funcArgStart);
			String funcArgs = filterClause
					.substring(
							funcArgStart + 1,
							funcArgEnd);
			funcArgs = funcArgs
			...
			if (Objects
				...
				final String afterFunc = filterClause
						.substring(
								funcArgEnd + 1);
				final String[] tokens = afterFunc
						.split(
								" ");
				...
				if (tokens.length < 2) {
					...
					if (!org.apache.commons.lang3.math.NumberUtils
							.isNumber(
									radiusStr)) {
						LOGGER
								.warn(
						...
						final Double r = org.apache.commons.lang3.math.NumberUtils
								.createDouble(
										radiusStr);
						if (r == null) {
							LOGGER
				...
				((GeomWithinDistance) pred.predicate)
						.setRadius(
								radius);
			}
			...
			final InputStoreInfo leftStore = inputStores
					.get(
							pred.leftTableRelation);
			final InputStoreInfo rightStore = inputStores
					.get(
			...
			final Index[] leftIndices = leftStore
					.getOrCreateAdapterIndexMappingStore()
					.getIndicesForAdapter(
							leftStore
									.getOrCreateInternalAdapterStore()
									.getAdapterId(
											leftStore.typeName))
					.getIndices(
							leftStore.getOrCreateIndexStore());
			final Index[] rightIndices = rightStore
					.getOrCreateAdapterIndexMappingStore()
					.getIndicesForAdapter(
							rightStore
									.getOrCreateInternalAdapterStore()
			...
			final Dataset<Row> leftFrame = leftResultFrame
					.getDataFrame(
							joinRunner.getLeftResults());
			final Dataset<Row> rightFrame = rightResultFrame
					.getDataFrame(
		...
		final Dataset<Row> results = session
				.sql(
						sql);
	...
	private String getTableNameFromArg(
		...
		final String[] attribSplit = funcArg
				.split(
						Pattern
								.quote(
										"."));
		...
		if (attribSplit.length == 2) {
			final InputStoreInfo storeInfo = inputStores
					.get(
							attribSplit[0].trim());
			if (storeInfo != null) {
	...
	private void loadStoresAndViews()
		...
		for (final InputStoreInfo storeInfo : addStores) {
					...
					.setQuery(
							QueryBuilder
									...
									.addTypeName(
											storeInfo.typeName)
									.build());
			storeInfo.rdd = GeoWaveRDDLoader
					.loadRDD(
							session.sparkContext(),
			...
			final Dataset<Row> dfTemp = dataFrame
					.getDataFrame(
							storeInfo.rdd);
			dfTemp
					.createOrReplaceTempView(
	...
	public String addInputStore(
		...
		if (addTypeName == null) {
			final List<String> adapterTypes = FeatureDataUtils
					.getFeatureTypeNames(
							storeOptions);
			final int adapterCount = adapterTypes.size();
			if (adapterCount > 0) {
				addTypeName = adapterTypes
						.get(
								0);
			}
			else {
